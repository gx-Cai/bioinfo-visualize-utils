...
def get_labels(data, fill=["number"], max_item_fill=30):
    """
    ...
    input
      ...
      fill: ["number"|"logic"|"percent"|"item"]
      max_item_fill: only valid when fill contains "item". The max number of items that shown in plot.
    ...
    if 'item' in fill:
        for k in set_collections:
            sep = sum([int(i) for i in k])
            sep = (2+N-sep)
            l = list(set_collections[k])
            if len(l) > max_item_fill: continue
...
def venn4(labels, names=['A', 'B', 'C', 'D'],show_text=True , **options):
    """
